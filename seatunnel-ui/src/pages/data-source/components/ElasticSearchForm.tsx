...
import { Button, Collapse, CollapseProps, Form, Input, InputNumber, Select, Space } from 'antd';
import { useEffect, useState } from 'react';

const ElasticSearchForm = ({ form }) => {
  useEffect(() => {}, []);
  ...
  const generateAutoName = () => {
    ...
    return `es_${datePart}_${timePart}`;
  ...
  const items: CollapseProps['items'] = [
    {
      ...
      children: (
        <div>
          <Form.Item
            label={<div style={{ height: 32, lineHeight: '33px' }}>驱动</div>}
            name="jdbcDriver"
          >
            <Select
              options={[
                {
                  label: 'x-pack-sql-jdbc-7.7.0.jar',
                  value: 'x-pack-sql-jdbc-7.7.0.jar',
                },
              ]}
              size="small"
            />
          ...
          <Form.Item
            label={<div style={{ height: 32, lineHeight: '33px' }}>Class</div>}
    ...
    },
    {
      ...
      children: (
        <div>
          <Form.Item
            label={<div style={{ height: 32, lineHeight: '33px' }}>使用SSH</div>}
            name="ssh"
          >
            <Select
              options={[
                    ...
                    <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                      ...
                      >
                        <Select
                          ...
                          options={[
                            {
                              label: 'useLocalTransactionState',
                              value: 'useLocalTransactionState',
                            },
                            { label: 'sslMode', value: 'sslMode' },
                          ]}
                        />
                      </Form.Item>
                      <Form.Item
                      ...
                      >
                        <Input placeholder="输入值" size="small" style={{ width: '290px' }} />
                      </Form.Item>
  ...
  return (
    <div style={{ padding: '0 16px' }}>
      <Form
        ...
        initialValues={{
          auth: 'NONE',
          jdbcDriverClass: 'org.elasticsearch.xpack.sql.jdbc.EsDriver',
          jdbcDriver: 'x-pack-sql-jdbc-7.7.0.jar',
          host: '127.0.0.1',
          port: '9200',
          ssh: 'false',
          environmentId: 1,
          alias: generateAutoName(),
          url: 'jdbc:es://127.0.0.1:9200/', // 初始值根据初始 host 和 port 设置
        }}
      ...
      >
        ...
        >
          <Input placeholder="jdbc:es://localhost:9200/" size="small" />
        </Form.Item>
