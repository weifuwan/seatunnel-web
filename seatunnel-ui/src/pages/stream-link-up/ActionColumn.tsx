...
import { seatunnelJobExecuteApi, seatunnelStreamJobDefinitionApi } from "./api";
...
const ActionColumn: React.FC<ActionColumnProps> = ({
  ...
  const doDeleteDataSource = async (id: string) => {
    const response = await seatunnelStreamJobDefinitionApi.delete(id);
    if (response.code === 0) {
      ...
      <Space size="middle">
        ...
        {record?.scheduleStatus === "RUNNING" ? (
          <Popconfirm
            ...
            onConfirm={async () => {
              if (record?.taskScheduleId) {
                const response = await taskScheduleApi.stopSchedule(
                  record?.taskScheduleId
                );
                if (response?.code === 0) {
                  cbk();
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              if (record?.taskScheduleId) {
                const response = await taskScheduleApi.startSchedule(
                  record?.taskScheduleId
                );
                if (response?.code === 0) {
                  cbk();
